...
const TermsScreen = () => {
  ...
  // Fetch Terms and Conditions from API with React Query
    ...
    return (
      <View style={{ flex: 1, backgroundColor: COLORS.background }}>
        ...
        <Header
          showBack={true}
          onBack={headerPress}
          title={t('termsAndConditions.title') || 'Terms & Conditions'}
        />
        <View style={styles.loadingContainer}>
          <ActivityIndicator size="large" color={COLORS.primary} />
          <Text style={styles.loadingText}>{t('termsAndConditions.loading') || 'Loading terms and conditions...'}</Text>
        </View>
      </View>
    );
  }
    ...
    return (
      <View style={{ flex: 1, backgroundColor: COLORS.background }}>
        ...
        <View style={styles.errorContainer}>
          ...
          <Text style={styles.errorTitle}>{t('termsAndConditions.errorTitle') || 'Unable to Load Terms & Conditions'}</Text>
          <Text style={styles.errorMessage}>
            {error?.message || t('termsAndConditions.errorMessage') || "We couldn't load the terms and conditions. Please check your connection and try again."}
          </Text>
  ...
  return (
    <View style={{ flex: 1, backgroundColor: COLORS.background }}>
      ...
      <Header
        showBack={true}
        onBack={headerPress}
        title={policyData?.title || t('termsAndConditions.title') || 'Terms & Conditions'}
      />
      <ScrollView
